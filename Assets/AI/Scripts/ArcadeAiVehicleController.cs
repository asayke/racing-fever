...
public class ArcadeAiVehicleController : Car
{
    ...
    //Ai stuff
    [HideInInspector] public float TurnAI = 1f;
    [HideInInspector] public float SpeedAI = 1f;
    [HideInInspector] public float brakeAI = 0f;
    ...
    private void Start()
    {
        ...
        if (movementMode == MovementMode.AngularVelocity) Physics.defaultMaxAngularSpeed = 100;
    }
            ...
            if (movementMode == MovementMode.AngularVelocity)
            {
                if (Mathf.Abs(SpeedAI) > 0.1f)
                    rb.angularVelocity = Vector3.Lerp(rb.angularVelocity,
                        carBody.transform.right * SpeedAI * MaxSpeed / radius, accelaration * Time.deltaTime);
            }
            else if (movementMode == MovementMode.Velocity)
            {
                if (Mathf.Abs(SpeedAI) > 0.1f && brakeAI < 0.1f)
                    rb.velocity = Vector3.Lerp(rb.velocity, carBody.transform.forward * SpeedAI * MaxSpeed,
                        accelaration / 10 * Time.deltaTime);
            }

            //body tilt
            carBody.MoveRotation(Quaternion.Slerp(carBody.rotation,
                Quaternion.FromToRotation(carBody.transform.up, hit.normal) * carBody.transform.rotation, 0.12f));
        }
        else
            carBody.MoveRotation(Quaternion.Slerp(carBody.rotation,
    ...
    public void Visuals()
    {
        ...
        //Body
        if (carVelocity.z > 1)
            BodyMesh.localRotation = Quaternion.Slerp(BodyMesh.localRotation, Quaternion.Euler(
                    Mathf.Lerp(0, -5, carVelocity.z / MaxSpeed),
                    BodyMesh.localRotation.eulerAngles.y, Mathf.Clamp(desiredTurning * TurnAI, -BodyTilt, BodyTilt)),
                0.05f);
        else
